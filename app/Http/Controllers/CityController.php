...
{
    // Save a city for the logged-in user
    public function saveCity(Request $request) {
        ...
        // Link the city to the user if it's not already linked
        ...
        // Redirect to the user's city list
        return redirect()->route('getCity');
    } 

    // Show all cities saved by the logged-in user
    public function getCity() {
        $userId = Auth::id();

        // Get all cities saved by the user with their details
    ...
    // Set a city as the user's favorite
    ...
    public function removeFavoriteCity(Request $request) {
        ...
        // Update the city so it is no longer a favorite
        UserPlaces::where('user_id', $user_id)
            ->where('place_id', $city_id)
            ->update(['is_favorite' => false]);
    ...
    // Enable daily weather updates for a city
    public function subscribeToDailyReport(Request $request) {
        $city_id = $request->input('subscribeToDailyReport');
        $user_id = Auth::id();
    
        // Set the city to send daily updates
        UserPlaces::where('user_id', $user_id)
            ->where('place_id', $city_id)
            ->update(['send_forecast' => true]);
    
        return redirect()->route('getCity')->with('status', 'You will receive regular updates!');
    } 

    // Disable daily weather updates for a city
    public function unsubscribeToDailyReport(Request $request) {
        $city_id = $request->input('unsubscribeToDailyReport');
        $user_id = Auth::id();
    
        // Set the city to stop sending daily updates
        UserPlaces::where('user_id', $user_id)
            ->where('place_id', $city_id)
            ->update(['send_forecast' => false]);
    
        return redirect()->route('getCity')->with('status', 'You will no longer receive updates!');
    } 
}
